...
def _load_store(path: _Path) -> Dict[str, Dict[str, Any]]:
    if path.exists():
        try:
            ...
            logger.warning("Failed to load store from %s", path, exc_info=True)
            return {}
    return {}


def _write_store(path: _Path, store: Dict[str, Dict[str, Any]]) -> None:
...
def _save_store(path: _Path, store: Dict[str, Dict[str, Any]]) -> None:
    with _STORE_LOCK:
        snapshot = copy.deepcopy(store)
        _write_store(path, snapshot)
...
def save_profile(user_id: str, profile: Dict[str, Any]) -> None:
    ...
    _save_store(_PROFILE_STORE_FILE, _profile_store)
...
def save_chat_conversation(conversation_id: str, conversation: Dict[str, Any]) -> None:
    ...
    _save_store(_CHAT_STORE_FILE, _chat_store)
...
def save_room(room_id: str, room: Dict[str, Any]) -> None:
    ...
    _save_store(_MESSAGING_STORE_FILE, _messaging_store)
...
def save_blind_game(game_id: str, game: Dict[str, Any]) -> None:
    ...
    _save_store(_BLIND_GAME_STORE_FILE, _blind_game_store)
...
def save_security_status(user_id: str, status: Dict[str, Any]) -> None:
    ...
    _save_store(_SECURITY_STORE_FILE, _security_store)
...
def save_trip(trip_id: str, trip: Dict[str, Any]) -> None:
    ...
    _save_store(_TRIP_STORE_FILE, _trip_store)
...
def save_post(post_id: str, post: Dict[str, Any]) -> None:
    ...
    _save_store(_COMMUNITY_STORE_FILE, _community_store)
