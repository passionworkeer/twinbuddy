...
try:
    from isolation import DataIsolation, UserSession, get_isolation
    _HAS_ISOLATION = True
except ImportError:
...
async def verify_session(
    ...
    """
    验证请求头中的 X-Session-Key 是否与 user_id 对应的 session_key 匹配。
    失败时抛出 403；session 不存在时抛出 404。
    """
    if di is None:
        raise HTTPException(status_code=503, detail="隔离层未启用")
    valid, session = di.verify_session_key(user_id, x_session_key)
    if session is None:
...
async def generate_avatar(
    ...
    表单字段直传（Form data）：
      ...
      - bio        自我介绍文本
      - chat_logs  聊天记录文本
      - user_id    匿名用户 ID（可选，不传则自动创建新会话）

    优先级：文件上传 > 表单字段（若传文件则忽略同名表单字段）

    数据隔离：
      - 上传文件存入 {user_id}/uploads/ 下（隔离目录）
      - 生成完成后原始上传文件被清除
      - Persona JSON 存入 {user_id}/persona.json
    ...
    """
    try:
        # Step 0: 获取或创建用户会话（数据隔离）
        isolation = get_isolation() if _HAS_ISOLATION else None
        session = isolation.get_or_create_session(user_id) if isolation else None
        ...
        # Step 1b: 若有文件上传，先存入隔离目录（供 engine 引用路径）
        final_photo = file_data["photo_path"]
        if photo_file and session and isolation:
            content_bytes = photo_file.file.read()
            photo_file.file.seek(0)
            stored = isolation.store_upload(session, photo_file.filename or "photo.jpg", content_bytes, subfolder="photos")
            final_photo = stored
            logger.info("用户文件已隔离存储 | user_id=%s | path=%s", session.user_id, stored)
        ...
        logger.info(
            ...
            "douyin_keys=%s | photo=%s | user_id=%s",
            final_mbti,
            len(final_bio),
            len(final_chat),
            list(final_douyin.keys()),
            final_photo,
            session.user_id if session else "N/A",
        )
        ...
        # Step 5: 存入隔离目录（数据隔离核心）
        if session and isolation:
            isolation.store_persona(session, persona)
            isolation.purge_uploads(session)  # 生成完毕，立即清除原始上传文件
            logger.info("Persona 已存入隔离目录 | user_id=%s", session.user_id)
        ...
        # Step 6: 返回成功响应（含 user_id 和 session_key，客户端后续请求需携带 X-Session-Key）
        return JSONResponse(
...
@app.delete("/user", response_model=UserResponse, tags=["用户数据"])
async def delete_user(
    ...
    请求头：
      - X-Session-Key: 必须在 generate_avatar 返回的 meta.session_key 中获取

    参数（Query）：
      - user_id: 匿名用户 ID（generate_avatar 返回的 meta.user_id）

    返回：
      - success: True 时 data 包含删除统计（deleted_files, total_files）
    """
    if di is None:
        return _build_error_response(
            HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="隔离层未启用")
        )
    valid, session = di.verify_session_key(user_id, x_session_key)
    if session is None:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    result = di.wipe(session)
    logger.info("用户数据已清除 | user_id=%s | files_deleted=%d", user_id, result["total_files"])
    return JSONResponse(
        status_code=status.HTTP_200_OK,
...
@app.get("/user/persona", response_model=UserResponse, tags=["用户数据"])
async def get_user_persona(
    ...
    返回：
      - success: True 时 data 包含 Persona JSON
    """
    if di is None:
        return _build_error_response(
            HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="隔离层未启用")
    ...
    if not valid:
        raise HTTPException(status_code=403, detail="Forbidden")
    persona = di.load_persona(session)
    if persona is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="该用户尚未生成 Persona，请先调用 /generate_avatar",
        )
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"success": True, "data": persona, "error": None},
    )


@app.get("/user/stats", response_model=UserResponse, tags=["系统"])
async def get_storage_stats() -> JSONResponse:
    """
    GET /user/stats

    返回当前存储统计信息（总用户数、总占用空间）。
    用于管理员监控，不暴露任何用户数据。
    """
    isolation = get_isolation() if _HAS_ISOLATION else None
    if not isolation:
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content={"success": True, "data": {"isolation_enabled": False}, "error": None},
        )
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"success": True, "data": isolation.get_storage_stats(), "error": None},
    )
