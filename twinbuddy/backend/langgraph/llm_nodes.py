...
def _get_negotiation_style(persona: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 也支持 _internal.cognition.negotiation_style
    if not neg and "_internal" in persona:
        neg = persona["_internal"].get("cognition", {}).get("negotiation_style", {})
    return neg
...
def _get_emotion_decision(persona: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    emo: Dict[str, Any] = persona.get("emotion_decision", {})
    if not emo and "_internal" in persona:
        emo = persona["_internal"].get("emotion", {})
    return emo
