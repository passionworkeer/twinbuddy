...
def _build_persona_block(
    ...
    # 标签与自述（用于将 onboarding 输入直接注入对话）
    raw_tags = persona.get("interest_tags", [])
    if not isinstance(raw_tags, list):
        raw_tags = []
    interest_tags = "、".join(str(x).strip() for x in raw_tags if str(x).strip()) or "未提供"
    voice_text = str(persona.get("voice_text", "") or "").strip() or "未提供"
...
def _llm_proposer(
    ...
    try:
        msg = PROPOSER_PROMPT.format(
            ...
            interest_tags=ub["interest_tags"],
            voice_text=ub["voice_text"],
...
def _llm_evaluator(
    ...
    try:
        msg = EVALUATOR_PROMPT.format(
            ...
            interest_tags=bb["interest_tags"],
            voice_text=bb["voice_text"],
            conversation_example=example,
            never_says=never_says,
            topic_label=topic_label,
